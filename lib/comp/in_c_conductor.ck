...
public class InCConductor extends Conductor {
  ...
  conf.put(DurationArg.make(MIN_REPEAT_PHRASE_DURATION, 2.0 * N.REST_1));
  ...
  conf.put(IntArg.make(PHASE_ADJ_DUR, PHASE_ADJ_NOTE_DUR));
  ...
  conf.put(IntArg.make(UNISON_PROB, 95));
  ...
  conf.put(IntArg.make(GAIN_ADJ_CRESCENDO_RATIO_THRESHOLD, 80));
  ...
  conf.put(FloatArg.make(GAIN_ADJ_DIMINUENDO_RATIO_THRESHOLD, 1.2));
  "GAIN_DIMINUENDO_ADJ_FACTOR" => GAIN_DIMINUENDO_ADJ_FACTOR;
  conf.put(IntArg.make(GAIN_DIMINUENDO_ADJ_FACTOR, 90));
  ...
  conf.put(IntArg.make(DIMINUENDO_PROB, 50));
  ...
  conf.put(FloatArg.make(TRANSPOSE_SHIFT, 1.0));
  ...
  fun void init(int numPhrases, int numPlayers, Sequence lastPhrase) {
    ...
    putGlobal(NUM_PLAYS_BEFORE_CONCLUDING_CRESCENDO,
              Math.random2(conf.MIN_LAST_PHRASE_REPETITIONS_BEFORE_CONCLUSION,
                           conf.MAX_LAST_PHRASE_REPETITIONS_BEFORE_CONCLUSION));

    Math.random2(conf.MIN_NUM_CONCLUDING_CRESCENDOS, conf.MAX_NUM_CONCLUDING_CRESCENDOS) => int numCrescendos;
    ...
    putGlobal(conf.NUM_CONCLUDING_CRESCENDOS, 2 * numCrescendos);
  ...
  // Override
  fun int isPlaying() {
    return !getGlobal(ALL_PLAYERS_STOPPED);
  }
  ...
  fun void instructionConclusionCrescendoDescrescento(int playerId) {
    ...
    if (!get(playerId, PLAYER_HAS_STOPPED) && reachedLastPhrase(playerId)) {
      ...
      if (ensembleReachedLastPhrase() && !inLastPhraseCrescendo) {
        ...
        get(playerId, PLAYER_LAST_PHRASE_BEFORE_CRESCENDO_PLAY_COUNT) => int playCount;
        ...
        get(playerId, PLAYER_LAST_PHRASE_CRESCENDO_PLAY_COUNT) => int playCount;
  ...
  fun /*private*/ int reachedLastPhraseCrescendo(int playerId) {
    get(playerId, PLAYER_LAST_PHRASE_BEFORE_CRESCENDO_PLAY_COUNT) == conf.NUM_PLAYS_BEFORE_CONCLUDING_CRESCENDO;
  }

  fun /*private*/ int advancePhraseIdx(int playerId) {
    return !reachedLastPhrase(playerId) && exceedsThreshold(conf.PHRASE_ADVANCE_PROB);
  }
  ...
  fun /*private*/ int repeatCurPhrase(Sequence seq, int curPhrasePlayCount) {
    ...
    return curPhrasePlayCount * seqDuration < conf.MIN_REPEAT_PHRASE_DURATION;
  }

  fun /*private*/ int seekingUnison(int playerId) {
    playersPhraseIdxRange() < conf.MAX_PHRASES_IDX_RANGE_FOR_SEEKING_UNISON => int inRange;
  ...
  fun /*private*/ int seekingCrescendo(int playerId) {
    ...
    assertFloatLessThan(ampRange, conf.MAX_AMP_RANGE_FOR_SEEKING_CRESCENDO) => int ampInMaxRange;
    return ampInMaxRange && exceedsThreshold(conf.CRESCENDO_PROB);
  }
 
  fun /*private*/ int seekingDiminuendo(int playerId) {
    ampRange() => float curAmpRange;
    assertFloatLessThan(ampRange, conf.MAX_AMP_RANGE_FOR_SEEKING_DIMINUENDO) => int ampInMaxRange;
    return ampInMaxRange && exceedsThreshold(conf.DIMINUENDO_PROB);
  }
  ...
  fun /*private*/ int rest(int playerId) {
    ...
    return exceedsThreshold(conf.REST_PROB_FACTOR * stayAtRestProbFactor);
  }

  fun /*private*/ int adjustPhase(int playerId) {
    conf.ADJ_PHASE_PROB => float adjPhaseProb;
    if (get(playerId, PLAYER_ADJ_PHASE_COUNT) <= conf.ADJ_PHASE_COUNT_THRESHOLD) {
      adjPhaseProb * conf.ADJ_PHASE_PROB_INCREASE_FACTOR => adjPhaseProb;
    }
  ...
  fun /*private*/ int tooFarBehind(int playerId) {
    ...
    return getAllMaxInt(PHRASE_IDX) - get(playerId, PHRASE_IDX) >= conf.PHRASES_IDX_RANGE_THRESHOLD;
  }

  fun /*private*/ int transpose(int playerId) {
    return exceedsThreshold(conf.TRANSPOSE_PROB_FACTOR);
  }
  ...
  fun float transposeFactor(int playerId) {
    ...
    if (exceedsThreshold(conf.TRANSPOSE_DOWN_PROB_FACTOR) && meanPhraseDuration >= conf.TRANSPOSE_DOWN_DUR_THRESHOLD) {
      return conf.TRANSPOSE_SHIFT_DOWN_FACTOR;
    } else {
      return conf.TRANSPOSE_SHIFT_UP_FACTOR;
    }
  }
